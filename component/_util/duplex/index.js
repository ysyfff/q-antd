    ...
    observer(
      class extends React.Component {
        componentDidMount() {
          ...
          // 检测code是否存在
          this.getKeyCodeFromProps((eventType, code) => {
            testKeyCode(code);
          });
          ...
          if (expanded) {
            const { key, value } = this.createKeyValueViaDot(
              duplex,
              this.expandedInitVal
            );
            model.set(key, value);
          }
        }
        ...
        componentWillUnmount() {
          ...
          const duplexString = Array.isArray(duplex) ? duplex[0] : duplex;
          this.props.duplexer.splice(
            this.props.duplexer.indexOf(duplexString),
            1
          );
        }
        ...
        isValidDuplexString(duplex) {
          const duplexString = Array.isArray(duplex) ? duplex[0] : duplex;
          return this.props.duplexer.every(
            n => !partOf(n, duplexString) && !partOf(duplexString, n)
          );
        }
        ...
        render() {
          ...
          warning(
            this.isValidDuplexString(duplex),
            'duplex的值不能是另一个duplex值得一部分，比如a和a.b不能出现在同一个form中'
          );
          // debugger
          ...
          if (dynamic) {
            ...
            duplexValue = this.getExpandedMapValue(
              duplex,
              mobx.toJS(model),
              this.expandedInitVal
            );
          ...
          warning(existed, `${duplex} 不存在model中`);
